function [sorted_predmat, sorted_truemat] = sm_TM_RBM_sortmat (sm_TM_RBM_predmat, sm_TM_RBM_truemat, is_dx)
...
%% Code
% If is_dx is true, the two matrices are sorted according to x-coordinates
    if is_dx == true
        sorted_predmat = sortrows(sm_TM_RBM_predmat, 1);
        sorted_truemat = sortrows(sm_TM_RBM_truemat, 1);
% If is_dx is false, the two matrices are sorted according to y-coordinates
    else
